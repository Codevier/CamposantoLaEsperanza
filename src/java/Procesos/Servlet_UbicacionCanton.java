 ...
 * @author Usuario
 */
public class Servlet_UbicacionCanton extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        Coneccion cone = null;
        cone= new Coneccion();
        String cantCode = req.getParameter("canton");
        String provCode = req.getParameter("provincia");
        String[][] ubicaciones= cone.Parroquias(cantCode,provCode);
        ...
        listaUbicacion.stream().map((Ubicacion) -> Json.createObjectBuilder()
                .add("productId", Ubicacion.ParroquiaCodigo)
                .add("purchaseCost", Ubicacion.UbicacionCodigo)
                .add("description", Ubicacion.ParroquiaNombre).build()).forEach((item) -> {
            array.add(item);
        });
